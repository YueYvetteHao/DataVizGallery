...
# Load pre-computed model artifacts from the pickle generated by code/export_model.py
@st.cache_data
def load_artifacts():
...
# Model registry: display name, artifact key prefix, and hex color for charts
MODELS = [
...
# Project description and biological interpretation
...
# Hide the delta arrow icon from metric cards (purely cosmetic)
st.markdown("<style>[data-testid='stMetricDelta'] svg { display: none; }</style>", unsafe_allow_html=True)

# ── Key metrics ───────────────────────────────────────────────────────────────
# Show mean ± std of 5-fold CV accuracy for each model, plus class count
cols = st.columns(5)
...
# ── Tab 1: ROC Curves ─────────────────────────────────────────────────────────
# One ROC panel per class (one-vs-rest), all 4 models overlaid on the same axes
with tab1:
    ...
    for col, cls in zip(roc_cols, labels):
        ...
        # Diagonal reference line representing random-chance performance (AUC = 0.5)
        fig_roc.add_trace(go.Scatter(
...
with tab2:
    # Bar chart comparing mean CV accuracy with ±std error bars across all models
    ...
    # Per-class precision / recall / F1 table for the selected model
...
with tab3:
    # 2×2 grid of heatmaps — one per model, colored by the model's assigned color
    row1 = st.columns(2)
    row2 = st.columns(2)
    ...
    # Grouped bar chart showing how accuracy varied across each of the 5 CV folds
...
with tab4:
    # Horizontal bar chart of top-N gene importances; hover shows gene description
    model_sel = st.radio(
