...
def plot_sv_barchart(intra_df):
    ...
    fig.update_layout(
        barmode="stack",
        title="Intra-chromosomal Structural Variation Events per Chromosome",
        xaxis=dict(title="Chromosome", tickfont=dict(size=9),
                   tickmode="array", tickvals=chroms, ticktext=chroms),
        yaxis=dict(title="Structural Variation Events", tickfont=dict(size=9)),
        legend=dict(title="SV Type", font=dict(size=9)),
        height=460,
